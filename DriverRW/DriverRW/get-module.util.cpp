...
PVOID GetModuleBase(PCHAR ModuleName) {
    ...
    NTSTATUS status = ZwQuerySystemInformation(
        SystemModuleInformation, &testSize, sizeof(ULONG64), &realLenght
    );

    if (status == STATUS_INFO_LENGTH_MISMATCH) {
        moduleBuff = ExAllocatePool(NonPagedPool, realLenght);
        if (!moduleBuff) {
            return 0;
        }
        status = ZwQuerySystemInformation(
            SystemModuleInformation, moduleBuff, realLenght, &realLenght
        );
        ...
        PRTL_PROCESS_MODULE_INFORMATION moduleArry = (PRTL_PROCESS_MODULE_INFORMATION)((ULONG64)moduleBuff + sizeof(ULONG64));
        for (size_t i = 0; i < modulesCount; i++) {
            PCHAR fullName = moduleArry->FullPathName;
            if (strstr(fullName, ModuleName)) {
                print("ÕÒµ½ÁË£¡%s\n", fullName);
                ExFreePool(moduleBuff);
                return moduleArry->ImageBase;
            }
            moduleArry++;
        }
    }
...
PUCHAR FindPatternRange(pv Start, ui32 size, const char* Pattern) {
    // find pattern utils
#define InRange(x, a, b) (x >= a && x <= b)
#define GetBits(x)       (InRange(x, '0', '9') ? (x - '0') : ((x - 'A') + 0xA))
#define GetByte(x)       ((UCHAR)(GetBits(x[0]) << 4 | GetBits(x[1])))
