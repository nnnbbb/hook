...
uint8_t read8_le(FILE *in) { return fgetc(in); }
...
void load_file(int argc, char *argv[]) {
  ...
  for (int idx = 1; idx < argc; idx++) {
    ...
    if (dosHeader.pe.signature != 0x4550) {
      printf(
          "invalid PE signature, file is likely corrupt PE, or not a valid PE "
          "file.\n");
      fclose(in);
      return;
    }
...
void print_headers(dos_header_t *dosHeader) {
  ...
  printf(" OpionalHeader Size:    %d (0x%X)\n",
         dosHeader->pe.optionalHeaderSize, dosHeader->pe.optionalHeaderSize);
