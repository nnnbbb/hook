...
ULONG PE::GetExportOffset(const unsigned char* FileData, ULONG FileSize, const char* ExportName) {
    ...
    if (pdh->e_magic != IMAGE_DOS_SIGNATURE) {
        Log("Invalid IMAGE_DOS_SIGNATURE!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    if (pnth->Signature != IMAGE_NT_SIGNATURE) {
        Log("Invalid IMAGE_NT_SIGNATURE!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    if (AddressOfFunctionsOffset == PE_ERROR_VALUE || AddressOfNameOrdinalsOffset == PE_ERROR_VALUE ||
        ...
        Log("Invalid Export Directory Contents!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    for (ULONG i = 0; i < NumberOfNames; i++) {
        ...
        if (CurrentNameOffset == PE_ERROR_VALUE) continue;
    ...
    if (ExportOffset == PE_ERROR_VALUE) {
        Log("Export %s not found in export table!\r\n", ExportName);
    }
