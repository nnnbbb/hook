...
static SSDTStruct* SSDTfind() {
    ...
    if (!SSDT) {
        ...
        if (kernelBase == 0 || kernelSize == 0) return nullptr;
        ...
        if (textSection == nullptr) return nullptr;

        // Find KiSystemServiceStart in .text
        const unsigned char KiSystemServiceStartPattern[] = {0x8B, 0xF8, 0xC1, 0xEF, 0x07, 0x83, 0xE7,
        ...
        for (KiSSSOffset = 0; KiSSSOffset < textSection->Misc.VirtualSize - signatureSize; KiSSSOffset++) {
            if (RtlCompareMemory(((unsigned char*)kernelBase + textSection->VirtualAddress + KiSSSOffset),
        ...
        if ((*(unsigned char*)address == 0x4c) && (*(unsigned char*)(address + 1) == 0x8d) &&
            (*(unsigned char*)(address + 2) == 0x15)) {
            relativeOffset = *(LONG*)(address + 3);
        }
        if (relativeOffset == 0) return nullptr;
...
PVOID SSDT::GetFunctionAddress(const char* apiname) {
    ...
    if (readOffset == -1) return 0;
    if (readOffset >= SSDT->NumberOfServices) {
        Log("Invalid read offset...\r\n");
        return 0;
    }
#ifdef _WIN64
...
static PVOID FindCaveAddress(PVOID CodeStart, ULONG CodeSize, ULONG CaveSize) {
    ...
    for (unsigned int i = 0, j = 0; i < CodeSize; i++) {
        if (Code[i] == 0x90 || Code[i] == 0xCC)  // NOP or INT3
            ...
            j = 0;
        if (j == CaveSize) return (PVOID)((ULONG_PTR)CodeStart + i - CaveSize + 1);
    }
    return 0;
}
#endif  //_WIN64
...
#ifdef _WIN64
    if (free) Hooklib::Unhook(hHook, true);
#else
    if (free) RtlFreeMemory(hHook);
#endif
}